...
public class ParticleContainer {
  ...
  public ParticleContainer(final int numberParticles, final Pose defaultPose,
      final SensorModel sensorModel, final MotionModel motionModel, final Map map,
      boolean useNoise) {
  ...
  public ParticleContainer(final List<Pose> poses, final SensorModel sensorModel,
      final MotionModel motionModel, final Map map) {
    this.particles = new ArrayList<>();
    this.sensorModel = sensorModel;
    this.motionModel = motionModel;
    this.map = map;
  ...
  public void updateAndResample(final SensorMessage sensorMessage, final int nthSample) {
    ...
    for (final WeightParticle weightParticle : particles) {
      final double newWeight =
          sensorModel.sampleSensor(sensorMessage, weightParticle.getParticle(), map, nthSample);
      ...
      for (WeightParticle wp : particles) {
        if (wp.weight < selection) {
          ...
          final Pose noiseyParticlePose = new Pose(origPose.getX() + Utils.randomN(0, 0.001),
              origPose.getY() + Utils.randomN(0, 0.001),
              origPose.getTheta() + Utils.randomN(0, 0.001));
